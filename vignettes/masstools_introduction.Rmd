...
You can use `masstools` to do the chemical formula operation. We will use functions from `r BiocStyle::CRANpkg("tidyverse")` for data manipulation in the following examples.
...
Now, `trans_ID` doesn't support vector query. So you can use the `r BiocStyle::CRANpkg("purrr")`'s `map()` function to achieve this.
...
# Interactive function: setwd_win() sets working directory on Windows by prompting for user input
setwd_win()
```
